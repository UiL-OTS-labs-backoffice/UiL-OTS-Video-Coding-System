...
;Configuration
  ...
  ;General
  Name "${AppName}"
  OutFile "${outLocation}/UiLOTS-VideoCodingSystem-${AppVersion}-Installer.exe"
...
Section "${AppName}" SecAppFiles
  ...
  ExecWait '$JREPath -jar "$INSTDIR/${JarFile}" setVLC="$VLCPath"'
SectionEnd
...
Section "Start menu shortcuts" SecCreateShortcut
  ...
  CreateShortCut "$SMPROGRAMS\${AppName}\${AppName}.lnk" "$JREPath" '-jar "$INSTDIR\${JarFile}"' "$INSTDIR\logo.ico" 0
; Etc
SectionEnd
